...
void* read_from_offset(const std::basic_string<charT> &file, unsigned long offset, size_t num_bytes, void *buffer, bool allow_alloc = true)
{
    ...
    if ((!allow_alloc) && buffer == NULL) {
        sp::util::sys::err::set_error(SP_ERR_INVALID_ADDRESS);
        return NULL;
    }
    ...
    if (buffer == NULL) {
        ...
        if ((buffer = CoTaskMemAlloc(num_bytes)) == NULL) {
            ...
            sp::util::sys::err::set_error(SP_ERR_OUT_OF_MEMORY);
            return NULL;
        }
    }
    ...
    if (!in_stream.good()) {
        ...
        sp::util::sys::err::set_error(SP_ERR_FILE_NOT_FOUND);
        return NULL;
    }
    ...
    if (in_stream.fail()) {
        ...
        if (manual_allocation)
            CoTaskMemFree(buffer);
        sp::util::sys::err::set_error(SP_ERR_INVALID_DATA);
        return NULL;
    }
    in_stream.close();
    return buffer;
